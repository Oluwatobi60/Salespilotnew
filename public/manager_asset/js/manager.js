document.addEventListener('DOMContentLoaded', function() {
  ...
  // Open side panel
  if (openAddStaffBtn && addManagerPanel && sidePanelOverlay) {
  ...
  function closePanel() {
    if (addManagerPanel) {
      addManagerPanel.classList.remove('active');
...
// Auto-generate Staff ID from surname (last word in fullname) and random 3 digits
  ...
  firstnameInput.addEventListener('input', generateStaffId);
  // If form is reset or loaded with value, generate
  if (firstnameInput.value) generateStaffId();
