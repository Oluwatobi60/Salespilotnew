...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Initialize Category and Supplier Panels
  console.log('🔧 Initializing Category and Supplier Panels...');

  // Initialize CategoryPanel if available
  if (typeof CategoryPanel !== 'undefined' && document.getElementById('category')) {
    try {
      CategoryPanel.init('category');
      console.log('✅ CategoryPanel initialized successfully');
    } catch (error) {
      console.error('❌ CategoryPanel initialization failed:', error);
    }
  } else {
    console.warn('⚠️ CategoryPanel not available. Type:', typeof CategoryPanel, 'Element:', document.getElementById('category'));
  }

  // Initialize SupplierPanel if available
  if (typeof SupplierPanel !== 'undefined' && document.getElementById('supplier')) {
    try {
      SupplierPanel.init('supplier');
      console.log('✅ SupplierPanel initialized successfully');
    } catch (error) {
      console.error('❌ SupplierPanel initialization failed:', error);
    }
  } else {
    console.warn('⚠️ SupplierPanel not available. Type:', typeof SupplierPanel, 'Element:', document.getElementById('supplier'));
  }
});
...
// ============================
// Initialize both Category and Supplier panels with retry mechanism
// ============================

// Initialize Category Panel Component with retry mechanism
(function initCategoryPanel() {
  console.log('CategoryPanel init - Starting retry mechanism...');
  ...
  const initInterval = setInterval(function () {
    attempts++;
    console.log(`CategoryPanel attempt ${attempts}/${maxAttempts}`);

    if (typeof CategoryPanel !== 'undefined' && document.getElementById('addCategoryPanel')) {
      console.log('CategoryPanel - Found! Initializing...');
      CategoryPanel.init('category');
      console.log('CategoryPanel - SUCCESS!');
      clearInterval(initInterval);
    } else if (attempts >= maxAttempts) {
      console.error('CategoryPanel - FAILED after ' + maxAttempts + ' attempts', {
        CategoryPanelDefined: typeof CategoryPanel !== 'undefined',
        PanelElement: document.getElementById('addCategoryPanel') !== null,
        CategorySelect: document.getElementById('category') !== null
      });
      clearInterval(initInterval);
    } else {
      console.log('CategoryPanel - Waiting...', {
        CP: typeof CategoryPanel !== 'undefined',
        Panel: document.getElementById('addCategoryPanel') !== null
      });
    }
  }, 100);
})();

// Initialize Supplier Panel Component with retry mechanism
(function initSupplierPanel() {
  console.log('SupplierPanel init - Starting retry mechanism...');
  ...
  const initInterval = setInterval(function () {
    attempts++;
    console.log(`SupplierPanel attempt ${attempts}/${maxAttempts}`);

    if (typeof SupplierPanel !== 'undefined' && document.getElementById('addSupplierPanel')) {
      console.log('SupplierPanel - Found! Initializing...');
      SupplierPanel.init('supplier');
      console.log('SupplierPanel - SUCCESS!');
      clearInterval(initInterval);
    } else if (attempts >= maxAttempts) {
      console.error('SupplierPanel - FAILED after ' + maxAttempts + ' attempts', {
        SupplierPanelDefined: typeof SupplierPanel !== 'undefined',
        PanelElement: document.getElementById('addSupplierPanel') !== null,
        SupplierSelect: document.getElementById('supplier') !== null
      });
      clearInterval(initInterval);
    } else {
      console.log('SupplierPanel - Waiting...', {
        SP: typeof SupplierPanel !== 'undefined',
        Panel: document.getElementById('addSupplierPanel') !== null
      });
    }
  }, 100);
})();
...
function saveVariantChanges() {
  ...
  if (costDisplay) costDisplay.textContent = 'â‚¦' + parseFloat(costPrice).toFixed(2);
  if (sellDisplay) sellDisplay.textContent = 'â‚¦' + parseFloat(sellingPrice).toFixed(2);
...
function editCalculateMarginPrice() {
  ...
  if (costPrice > 0 && targetMargin > 0) {
    ...
    if (editMarginFinalPrice) {
      editMarginFinalPrice.textContent = 'â‚¦' + finalPrice.toFixed(2);
    }
  } else {
